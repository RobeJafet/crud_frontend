...
$(document).on('click', function (event) {
  // Check if the clicked element is not inside '.container-button' or any of its children
  if (!$(event.target).closest('.container-button').length) {
    // If not, remove the 'active' class from '.container-button'
...
$(document).on('click', '.btn-editar', function (event) {
  ...
  $.ajax({
    ...
    success: function (response) {
      $('#input-name-edit').val(response.nombre);
      $('#input-email-edit').val(response.email);
      $('#input-tel-edit').val(response.telefono);
      $('#select-input-edit')[0].selectize.setValue(response.rol);
    },
